...
-- 다중 행 연산자 (IN, ANY, ALL)
-- IN: 조회된 목록의 어떤 값과 같은지를 확인합니다
SELECT * FROM employees
WHERE job_id IN (SELECT job_id FROM employees
                WHERE job_id = 'IT_PROG');

-- first_name이 David인 사람들의 급여과 같은 급여를 받는 사람들을 조회
SELECT * FROM employees
...
-- ALL: 같은 서브쿼리에 의해 리턴된 각각의 값과 모두 비교해서
-- 모두 만족해야 합니다.
...
WHERE ROWNUM > 21 AND ROWNUM  <= 30; -- 0보다 크다 라고 설정하면 왜 실행이 되는걸까
