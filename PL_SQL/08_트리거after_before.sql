...
트리거 자체를 트랜잭션의 일부로 처리하기 때문에 COMMIT이나 ROLLBACK을 포함할 수 없습니다.
*/
...
CREATE OR REPLACE TRIGGER trg_user_insert
    ...
    FOR EACH ROW
BEGIN
    :NEW.name := SUBSTR(:NEW.name, 1, 1) || '****'; -- 앞글자 한개만 따오고 나머지는 *로 표시하겠다는 뜻
END;
...
BEGIN
    ...
    EXCEPTION
        WHEN quantity_shortage_exception THEN
            ...
            -- 첫 번째 매개값: 에러 코드 (사용자 정의 예외 -20000 ~ -20999까지)
            -- 두 번째 매개값: 에러 메세지
...
-- 예외가 발생하여 insert가 동작되지 않았을 때 ROLLBACK이 됨
