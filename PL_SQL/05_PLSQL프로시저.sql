...
CREATE PROCEDURE guguproc
    (p_dan IN NUMBER)
IS -- DECLARE는 변수 선언할 내용이 없으면 지워도 되지만 IS는 불가
BEGIN
...
-- 매개값(인수) 없는 프로시저
CREATE PROCEDURE p_test -- 값을 전달받지 않음
IS -- 선언부
...
BEGIN
    ...
    my_new_job_proc('CEO', 'test_ceo', 20000, 8000, msg);
    dbms_output.put_line(msg);
    
    my_new_job_proc('CEO2', 'test_ceo2', 20000, 8000, msg);
    dbms_output.put_line(msg);
END;
...
BEGIN
    ...
    dbms_output.put_line('END 전 p_var2: '|| p_var2); -- OUT: 값이 전달되지 않음
