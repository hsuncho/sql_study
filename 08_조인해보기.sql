...
-- 조인 조건과 일치하지 않는 테이터는 조회 대상에서 제외
...
SELECT
    ...
    d.department_id, e.job_id, j.job_title, loc.city
FROM 
    employees e,
    departments d,
    jobs j,
...
2. location_id 값과 같은 값을 가지는 데이터를 departmnets에서 찾아서 조인
3. 위의 조인한 결과와 동일한 departments_id를 가진 employees 테이블의 데이터를 찾아 조인
...
WHERE e.department_id = d.department_id(+)
AND d.location_id = loc.location_id; --내부조건
